...
# This file is copied from PR 5940 in PyTorch Geometric only for development
# purposes and will be removed as soon as the PR is merged.
...
from poptorch_geometric.pyg_pad import Pad
from poptorch_geometric.pyg_pad import (
    AttrNamePadding,
    EdgeTypePadding,
    NodeTypePadding,
    Padding,
...
def _check_homo_data_nodes(original: Data,
                           padded: Data,
...
def _check_homo_data_edges(original: Data,
                           padded: Data,
...
def _check_hetero_data_nodes(original: HeteroData,
                             padded: HeteroData,
                             max_num_nodes: Union[int, Dict[NodeType, int]],
                             node_pad_value: Optional[Padding] = None,
                             exclude_keys: Optional[List[str]] = None) -> None:
...
def _check_hetero_data_edges(
    ...
    for edge_type, attr in _generate_heterodata_edges(padded):
        ...
        if attr == 'edge_index':
            ...
            assert all(i == pad_value for i in torch.flatten(
                padded_tensor[compare_pad_start_idx:, :]))
