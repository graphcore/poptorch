...
def test_gen_conv(aggr, dataset):
    pytest.skip('AFS-187 Precision errors requires investigations')
    in_channels = dataset.num_node_features

    conv = GENConv(in_channels, 32, aggr, edge_dim=16, add_self_loops=False)
    ...
    conv = GENConv((in_channels, in_channels), 32, aggr, add_self_loops=False)
    conv_harness(conv, dataset, batch=batch)
